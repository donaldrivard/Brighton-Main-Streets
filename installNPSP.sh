...
sfdx force:package:install -i 04t80000000gZsgAAE -w 100
if [ "$?" = "0" ]; then
...
sfdx force:mdapi:deploy -d Cumulus-master/unpackaged/pre/opportunity_record_types/
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t1Y0000011SkhQAE -w 100 
if [ "$?" = "0" ]; then
