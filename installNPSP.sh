...
sfdx force:package:install -i 04t80000001AWvwAAG -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t80000000y8tyAAA -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t80000000gZsgAAE -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t80000000y8kRAAQ -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t80000000lTMlAAM -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
unzip master.zip
sfdx force:mdapi:deploy -d Cumulus-master/unpackaged/pre/account_record_types/ -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:mdapi:deploy -d Cumulus-master/unpackaged/pre/opportunity_record_types/ -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t1Y0000011SkhQAE -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
...
sfdx force:package:install -i 04t50000000cSq7AAE -w 100 -u $TARGET_SCRATCH_ORG_ALIAS
if [ "$?" = "0" ]; then
	echo 'Installation of Package 6 complete...'
else
	echo 'Installation failed.'
